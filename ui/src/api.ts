...
export const api = {
  ...
  productAdmins: (pk: string) => req<{ admins: { email: string; name: string }[]; super_admins: { email: string; name: string }[] }>('GET', `/v1/products/${pk}/admins`),
  ...
  settingsGet: (pk: string) => req<{ similarity_threshold: number; registry_default: number; overridden: boolean }>('GET', `/v1/products/${pk}/settings`),
